...
# read the image
img_color = cv2.imread("opencv/sample_images/9Osteosarcoma_01.tif")
img = img_color[:,:,0]
pixel_size_um = 0.454
...
# color each grain
img_cell_color = color.label2rgb(markers, bg_label=0)


cv2.imshow("watershed", img_color)
cv2.imshow("colored grains",img_cell_color)
cv2.waitKey(0)

cv2.imwrite("opencv/processed_image/9Cell_Analysis/9watershed.jpg", img_color)
...
output_file = open("opencv/processed_image/9Cell_Analysis/9image_measurements.csv", "w")
